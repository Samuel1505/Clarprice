...
describe('Prediction Market Contract', () => {
  ...
  it('allows users to bet on outcomes', () => {
    ...
    // @ts-ignore
    const marketData = market.result.value.value;
  ...
  it('handles resolution and claiming with streaks', () => {
    ...
    // 1. Create Market
    const create = simnet.callPublicFn(contract, "create-market", [Cl.stringAscii("WORLD"), Cl.stringAscii("Election?"), Cl.stringAscii("X"), Cl.stringAscii("Y"), Cl.uint(10000)], deployer);
    // @ts-ignore
    const marketId = create.result.value;

    // 2. Bet: Wallet1 bets 1000 on Win, Wallet2 bets 1000 on Loss
    simnet.callPublicFn(contract, "place-bet", [marketId, Cl.stringAscii("X"), Cl.uint(1000)], wallet1);
    simnet.callPublicFn(contract, "place-bet", [marketId, Cl.stringAscii("Y"), Cl.uint(1000)], wallet2);

    // 3. Resolve Market (Winner X)
    const resolve = simnet.callPublicFn(contract, "resolve-market", [marketId, Cl.stringAscii("X")], deployer);
    ...
    const claim1 = simnet.callPublicFn(contract, "claim-winnings", [marketId], wallet1);
    expect(claim1.result).toBeOk(Cl.bool(true));

    const stats1 = simnet.callReadOnlyFn(contract, "get-user-stats", [Cl.standardPrincipal(wallet1)], deployer);
    ...
    const claim2 = simnet.callPublicFn(contract, "claim-winnings", [marketId], wallet2);
    expect(claim2.result).toBeOk(Cl.bool(true));

    const stats2 = simnet.callReadOnlyFn(contract, "get-user-stats", [Cl.standardPrincipal(wallet2)], deployer);
  ...
  it('calculates streak multipliers correctly', () => {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      simnet.callPublicFn(contract, "claim-winnings", [marketId], wallet1);
    }
    ...
    const finalStats = simnet.callReadOnlyFn(contract, "get-user-stats", [Cl.standardPrincipal(wallet1)], deployer);
